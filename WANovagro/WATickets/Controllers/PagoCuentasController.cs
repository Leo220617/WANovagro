...
namespace WATickets.Controllers
{
    ...
    public class PagoCuentasController : ApiController
    {
        ...
        [Route("api/PagoCuentas/SincronizarRestante")]
        [HttpPost]
        public HttpResponseMessage SincronizarRestante([FromBody] PagoCuentas cuenta)
        {
            var t = db.Database.BeginTransaction();
            ...
            try
            {
                ...
                Pagos = Pagos.Where(a => a.idCaja == 0 && a.idCajero == 0 && a.Fecha == time && a.MonedaVuelto == "CRC" && a.Moneda == "USD" && a.Metodo == "Efectivo" && (a.PagadoCon - a.Monto) > 0).ToList();
                var Monto = 0.0m;
                var PagadoCon = 0.0m;
                var Total = 0.0m;
                ...
                {
                    PagoCuenta = new PagoCuentas();
                    PagoCuenta.idCliente = 0;
                    PagoCuenta.CodSuc = "001";
                    PagoCuenta.Fecha = DateTime.Now;
                    PagoCuenta.FechaVencimiento = DateTime.Now;
                    PagoCuenta.FechaContabilizacion = DateTime.Now;
                    PagoCuenta.Comentarios = "Pago a Cuenta restante vueltos";
                    PagoCuenta.Total = Total;
                    PagoCuenta.Moneda = "USD";
                    PagoCuenta.idUsuarioCreador = 0;
                    PagoCuenta.idCaja = 0;
