...
{
    public class MetodosPagosCuentasController : ApiController
    {
        ...
        [Route("api/MetodosPagosCuentas/Consultar")]
        public HttpResponseMessage GetOne([FromUri] int id)
        {
            try
            {
                MetodosPagosCuentas MetodosPagosCuentas = db.MetodosPagosCuentas.Where(a => a.id == id).FirstOrDefault();


                return Request.CreateResponse(System.Net.HttpStatusCode.OK, MetodosPagosCuentas);
            }
            catch (Exception ex)
            {
                BitacoraErrores be = new BitacoraErrores();
